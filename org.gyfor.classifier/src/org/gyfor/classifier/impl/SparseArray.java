...
/**
 * SparseMap map a key object to integers. Unlike a normal array of integers,
 * there can be gaps in the indices. It is intended to be more efficient than
 * using a HashMap to map objects to Integers.
 */
public class SparseArray<S extends Comparable<S>> implements Cloneable {
  ...
  public SparseArray (int initialCapacity) {
    ...
    mKeys = (S[])new Object[initialCapacity];
  ...
  @SuppressWarnings("unchecked")
  @Override
  public SparseArray<S> clone() {
    SparseArray<S> clone = null;
    try {
      clone = (SparseArray<S>)super.clone();
   ...
   */
  public int get(S key, int valueIfKeyNotFound) {
   ...
   */
  public void put(S key, int value) {
    int i = binarySearch(mKeys, 0, mSize, key);
   ...
   */
  public void append(S key, int value) {
    if (mSize != 0 && key.compareTo(mKeys[mSize - 1]) <= 0) {
      put(key, value);
      return;
    }
  ...
  private int binarySearch(S[] a, int start, int len, S key) {
    int high = start + len;
    int low = start - 1;
    int guess;
